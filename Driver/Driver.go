...
func main() {
	...
	router.HandleFunc("/api/v1/driver/view/", viewdriver).Methods("GET")          // view and get all the drivers available in the database.
	router.HandleFunc("/api/v1/driver/{driverid}", driver).Methods("POST", "PUT") // create new and update existing driver in database

	// Driver Status and Trip related
	router.HandleFunc("/api/v1/driver/status/online/{driverid}", driveronline).Methods("GET", "PUT")   //change the driver status to Available
	router.HandleFunc("/api/v1/driver/status/offline/{driverid}", driveroffline).Methods("GET", "PUT") // change the driver status to Busy
	router.HandleFunc("/api/v1/drivers/", autoassigndriver)                                            // Chooses a single random driver from the Driver table which has the status Available

	router.HandleFunc("/api/v1/driver/trips/{driverid}", getdrivertrip).Methods("GET") // get the trips assigned to a specific driver

	router.HandleFunc("/api/v1/driver/trips/start/{tripid}", begintrip).Methods("PUT")           //start trip and change trip status to Ongoing
	router.HandleFunc("/api/v1/driver/trips/end/{tripid}/{driverid}", finishtrip).Methods("PUT") //end trip and change trip status to Completed and driverstatus to Busy
...
// Retrieve Driver - Gets all the drivers from the database and store in Driver struct
		...
		if body, err := ioutil.ReadAll(r.Body); err == nil {
			...
			if err := json.Unmarshal(body, &data); err == nil {
				...
				if dExist {
					...
					fmt.Println("Driver Exists, Updating Status to Busy")
					w.WriteHeader(http.StatusAccepted) //202
...
func insertDriver(driverid string, d Driver) {
	...
	_, err = db.Exec("INSERT into Driver values(?,?,?,?,?,?,?,?)", driverid, d.FirstName, d.LastName, d.MobileNumber, d.Email, d.CarLicense, d.IcNo, status)
	if err != nil {
		fmt.Println("failed to connect to insert")
		fmt.Println(err)
	} else {
...
func driverongoing(driverid string) (Trip, bool) {
	...
	results := db.QueryRow("Select * from trip where driverid =? AND TripStatus != 'Completed'", driverid)
