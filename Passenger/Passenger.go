...
// Empty Maps Created to store information.
...
func main() {
	...
	router.HandleFunc("/api/v1/passenger/view/", viewpassenger).Methods("GET")             //gets the list of all passengers in the database
	router.HandleFunc("/api/v1/passenger/{passengerid}", passenger).Methods("POST", "PUT") // create new passenger and update existing passenger to database

	// Passenger Trip Related
	router.HandleFunc("/api/v1/passenger/trip/{tripid}/{passengerid}/{driverid}", newtrip).Methods("GET", "POST") // create new trip request, add new trip record to table if driver is assigned
	router.HandleFunc("/api/v1/trip/{passengerid}", viewpassengertrip).Methods("GET")                             // get passenger trip history in reverse chronological order
	...
	if r.Method == "POST" {
		...
		if body, err := ioutil.ReadAll(r.Body); err == nil {
			...
			if err := json.Unmarshal(body, &data); err == nil {
				...
				//if passenger id exist, update, if not, don't update
				_, pExist := passengerExist(params["passengerid"])
	...
	if r.Method == "POST" {
		if body, err := ioutil.ReadAll(r.Body); err == nil {
			...
			if err := json.Unmarshal(body, &data); err == nil {
				...
				//if passenger exist, if passenger is in an ongoing trip
				_, pExist := passengerExist(params["passengerid"])
				...
				if pExist {
					...
					if !pExistOngoing {
						...
						//passenger is already in an ongoing trip and trip is not created
...
func viewpassengertrip(w http.ResponseWriter, r *http.Request) {
	...
	//get all the trips for a specific passenger id, then order it by tripdate in reverse chronological order
...
// - check if passenger exist, take in passenger id and return bool
...
// - check if passenger is in an ongoing trip, take in passenger id and return bool
...
// - create new passenger, take in passengerid and Passenger struct
func insertPassenger(passengerid string, p Passenger) {
	...
	_, err = db.Exec("INSERT into Passenger values(?,?,?,?,?)", passengerid, p.FirstName, p.LastName, p.MobileNumber, p.Email)
	if err != nil {
		fmt.Println("Error")
		fmt.Println(err)
	} else {
...
// - update existing passenger, take in passengerid and Passenger struct
...
// - create new trip request, take in tripid, passengerid, driverid, and postal codes for Trip struct and create new trip in Trip table
